...
parse = reqparse.RequestParser()  # 拿到信息
parse.add_argument('uname', type=str, required=True, help='请提供用户名')
parse.add_argument('uemail', type=str, help='请提供用户邮箱')
parse.add_argument('upassword', type=str, required=True, help='请提供用户密码')
parse.add_argument('action', type=str, required=True, help='请申明具体操作')
...
class UserResource(Resource):
    ...
    def post(self):
        ...
        if action == 'register':
            ...
            subject = "TPP用户激活"
            msg = Message(subject=subject, sender='664490254@qq.com', recipients=[u_email])
            html = render_template('user_activate.html', username='Jeffrey',
